...
class UserController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->role->name != 'admin') {
            ...
            if (
                isset($request->user_belongstomany_role_relationship)
                && in_array(1, $request->user_belongstomany_role_relationship)
            ) {
                if (($keyArray = array_search('1', $request->user_belongstomany_role_relationship)) !== false) {
                    $updatedData = $request->user_belongstomany_role_relationship;
                    $updatedData[$keyArray] = null;
                    $request->merge([
                        "user_belongstomany_role_relationship" => $updatedData,
                    ]);
                }
            }
        }
