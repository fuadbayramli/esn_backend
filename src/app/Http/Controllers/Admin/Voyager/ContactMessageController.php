...
class ContactMessageController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            $dataTypeContent->update(['status' => 1]);
        } else {
            // If Model doest exist, get data from table name
            $dataTypeContent = DB::table($dataType->name)->where('id', $id)->first()->update(['status' => 1]);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($request->answer)) {
            $to_email = $data->email;
            $to_name = $data->name;
            Mail::send('mail', ['data' => $request->only(['answer'])], function ($message) use ($to_email, $to_name) {
